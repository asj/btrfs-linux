...
static void dump_file_stat(const char *devpath)
{
	struct path path;
	struct kstat stat;
	int err;
	...
	if (err) {
		pr_err("ASJ: kern_path failed for %s, error: %d\n", devpath, err);
		return;
	}

	err = vfs_getattr(&path, &stat, STATX_BASIC_STATS, AT_STATX_SYNC_AS_STAT);
	if (err) {
		pr_err("ASJ: vfs_getattr failed, error: %d\n", err);
		path_put(&path);
		return;
	}

	if (S_ISREG(stat.mode))
		pr_info("ASJ: Type: Regular file\n");
	else if (S_ISDIR(stat.mode))
		pr_info("ASJ: Type: Directory\n");
	else if (S_ISCHR(stat.mode))
		pr_info("ASJ: Type: Character device\n");
	else if (S_ISBLK(stat.mode))
		pr_info("ASJ: Type: Block device\n");
	else if (S_ISFIFO(stat.mode))
		pr_info("ASJ: Type: FIFO/pipe\n");
	else if (S_ISLNK(stat.mode))
		pr_info("ASJ: Type: Symlink\n");
	else if (S_ISSOCK(stat.mode))
		pr_info("ASJ: Type: Socket\n");
	else
		pr_info("ASJ: Type: Unknown (mode: 0x%x)\n", stat.mode);

	if (S_ISBLK(stat.mode))
		pr_info("ASJ: This IS a block device. %s \n", devpath);
	else
		pr_info("ASJ: This is NOT a block device. %s \n", devpath);
...
#define BTRFS_SEQ_PRINT2(plist, arg) \
	...
	list_for_each(cur_uuid, fs_uuids) {
		...
		BTRFS_SEQ_PRINT2("\tmount_opt:\t\t%llx\n", fs_info->mount_opt);
			   ...
			   struct btrfs_fs_devices *sprt)
{
	...
	BTRFS_SEQ_PRINT("\t\tdevice:\t\t%s\n",
			device->name ? rcu_dereference(device->name) : "(null)");
	rcu_read_unlock();
	...
	if (device->name)
		dump_file_stat(rcu_dereference(device->name));
}
...
//	BTRFS_SEQ_PRINT("\tfsid_change:\t\t%d\n", fs_devices->fsid_change);
	...
	BTRFS_SEQ_PRINT("\tlatest_dev:\t\t%s\n", fs_devices->latest_dev ?
			rcu_dereference(fs_devices->latest_dev->name) : "NULL");
	rcu_read_unlock();
#ifndef NO_READPOLICY
